...
{	
	...
	for each (Primitive *prim in m_sceneObjects) // enumerating through each object
	{
		RayHitResult newHit = prim->IntersectByRay(ray); //intersection check with the given ray
		if (newHit.t < result.t && newHit.data != NULL) // checks if I'm hitting something and if the hit result it's the closest one to the start of the ray
		{
			if (newHit.t >= 0) // we are only interested in t values that are NOT behind the origin of the ray 
			{
				result = newHit;
			}
		}
